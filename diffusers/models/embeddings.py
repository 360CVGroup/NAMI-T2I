...
def get_1d_rotary_pos_embed(
    ...
    if stages_index == 0:
        linear_factor = ntk_factor
        ntk_factor = 1.0
    else:
        linear_factor = 1.0
    theta = theta * ntk_factor #pow(ntk_factor,float(dim)/(dim-2.0))
...
class FluxPosEmbed(nn.Module):
    ...
    def __init__(self, theta: int, axes_dim: List[int], ntk_factor=1.0, stages_index=-1):
        super().__init__()
        self.theta = theta
        self.axes_dim = axes_dim
        self.ntk_factor = ntk_factor
        self.stages_index = stages_index
    def forward(self, ids: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(n_axes):
            cos, sin = get_1d_rotary_pos_embed(
                self.axes_dim[i], pos[:, i], ntk_factor=self.ntk_factor, stages_index=self.stages_index, repeat_interleave_real=True, use_real=True, freqs_dtype=freqs_dtype
            )
            cos_out.append(cos)
            sin_out.append(sin)
...
class CombinedTimestepTextProjEmbeddings(nn.Module):
    ...
    def forward(self, timestep, pooled_projection,if_use_pool=True):
        timesteps_proj = self.time_proj(timestep)
...
class CombinedTimestepGuidanceTextProjEmbeddings(nn.Module):
    ...
    def forward(self, timestep, guidance, pooled_projection,if_use_pool=True):
        timesteps_proj = self.time_proj(timestep)
        timesteps_emb = self.timestep_embedder(timesteps_proj.to(dtype=pooled_projection.dtype))  # (N, D)
