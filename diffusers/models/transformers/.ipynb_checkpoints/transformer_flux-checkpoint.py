...
class FluxTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        stage_singleblock_index: Tuple[int] = (0, 19, 38),
        ntk_factor=1.0,
    ):
        super().__init__()
        ...
        self.pos_embed_0 = FluxPosEmbed(theta=10000, axes_dim=axes_dims_rope, ntk_factor=ntk_factor, stages_index=0)
        self.pos_embed = FluxPosEmbed(theta=10000, axes_dim=axes_dims_rope, ntk_factor=ntk_factor)
        self.stage_singleblock_index = stage_singleblock_index
    ...
    def forward(
        ...
        if guidance is not None:
            ...
            guidance = None
        temb = (
            self.time_text_embed(timestep, pooled_projections,if_use_pool)
            if guidance is None
            else self.time_text_embed(timestep, guidance, pooled_projections,if_use_pool)
        )
        ...
        if stage_index== 0:
            image_rotary_emb = self.pos_embed_0(ids)
        else:
        ...
        for index_block, block in enumerate(self.single_transformer_blocks):
            if stage_index != -1:
                if index_block == self.stage_singleblock_index[stage_index]:
                    #print (stage_index, self.stage_singleblock_index[stage_index])
                    break
            if self.training and self.gradient_checkpointing:
